...
macro(oclm_find_package _name)
  ...
  if(${name}_DISABLE)
    ...
    set(${name}_ROOT ${${name}_ROOT} CACHE INTERNAL "${name} root directory.")
    set(${name}_LIBRARY ${${name}_LIBRARY} CACHE INTERNAL "${name} shared library.")
    ...
    if(NOT ${name}_SEARCHED)
      ...
      if(${name}_FOUND)
        ...
        set(${name}_ROOT ${${name}_ROOT} CACHE ADVANCED "${name} root directory.")
        set(${name}_LIBRARY ${${name}_LIBRARY} CACHE ADVANCED "${name} shared library.")
        set(${name}_LIBRARY_DIR ${${name}_LIBRARY_DIR} CACHE ADVANCED "${name} library directory.")
        set(${name}_INCLUDE_DIR ${${name}_INCLUDE_DIR} CACHE ADVANCED "${name} include directory.")
      else()
        if(NOT ${name}_ROOT)
          set(${name}_ROOT ${name}_ROOT-NOTFOUND)
        endif()
